...
"""
global celdaVacia, memoria, k1, k2, objetoRecogido  #k1 equivale a k+ en la función(recoger) y k2 equivale a k- en la función(soltar_LF)
global agente
celdaVacia = -7
agente = -100
k1 = 1
...
"""
Función para desplazar_LF elementos en el arreglo Memoria
"""
def desplazar_LF (memoria, elementoRecogido, cantPasos):
...
"""
def dejarObjeto_LF (Matriz, i, j, auxi, auxj, elemento):
    if (Matriz [i][j] == celdaVacia):
        Matriz [i][j] = elemento
...
"""
def valorF_LF(elemento, df, memoria, cantPasosTotal, cantPasos):
    if (elemento >= 0):
        ...
        if (cantPasosTotal < cantPasos):
            ...
            desplazar_LF(memoria, dato[0][0], cantPasos)
        ...
        if (cantPasosTotal < cantPasos):
            ...
            desplazar_LF(memoria, elemento, cantPasos)
        valoresUnicos, contadorOcurrencias = np.unique(memoria, return_counts=True)
        maximo = -1
        posicionMaximo = -1
        for n in range(contadorOcurrencias.size):
        ...
        return f
"""
Función p(recoger_LF) = (k1/(k1 + f))^2
"""
def recoger_LF (k1, f):
    pRecoger = ((k1/(k1 + f))**2)
    return pRecoger
"""
Función p(soltar_LF) = (f/(f - k2))^2
"""
def soltar_LF (k2, f):
    pSoltar = ((f/(f + k2))**2)
    return pSoltar
...
"""
def recorrer_LF (Matriz, caminos, fila, cantAgentes, cantPasos, df, c):
    fig, ax = plt.subplots()
    a = 0
    ...
    for m in range(cantAgentes):
        ...
        if (Matriz[i][j] == agente):
            ...
            for b in range (cantPasos):
                ...
                f = valorF_LF(valorEvaluar, df, memoria, cantPasosTotal, cantPasos)
                if ((Matriz[ni][nj] >= 0) & (objetoRecogido == -1)):
                    recogerDato = recoger_LF(k1, f)
                    if (recogerDato >= u1):
                    ...
                    soltarDato = soltar_LF(k2, f)
                    if (soltarDato >= u2):
                        print("soltarDato", soltarDato, "     valor de f:   ", f, "     umbral 2:   ", u2)
                        dejarObjeto_LF(Matriz, ni, nj, auxi, auxj, objetoRecogido)
                        objetoRecogido = -11
            ...
            #    print("Valor de f: ", f,)
                ...
                if (b == cantPasos-1):
                    dejarObjeto_LF(Matriz, fila, c, auxi, auxj, agente)
